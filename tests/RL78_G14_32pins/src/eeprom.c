...
void EE_WriteEEBlock(uint16_t start_address, uint16_t length, uint8_t const *pucBuffer)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS, pack, length + 2, 100);
}
...
void EE_WriteWord(uint16_t WordAddress, uint16_t Word)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS, pack, 4, 100);
    while (--Delay);
}
...
void EE_WriteByte(uint16_t ByteAddress, uint8_t Byte)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS, pack, 3, 100);
}
...
void EE_ReadEEBlock(uint16_t start_address, uint16_t length, uint8_t *pucBuffer)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS, pack, 2, 100);
    /* Wait to receive IIC reply from the EEP */
    while(R_IIC20_Master_Receive(AT24C32_ADDRESS, pack, length, 100) != MD_OK);
...
uint16_t EE_ReadWord(uint16_t WordAddress)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS,(uint8_t *) &WordAddress, 2, 100);

    /* Wait to receive IIC reply from the EEP */
    while(R_IIC20_Master_Receive(AT24C32_ADDRESS, (uint8_t *) &Word, 2, 100) != MD_OK);
...
uint8_t EE_ReadByte(uint16_t ByteAddress)
{
    ...
    R_IIC20_Master_Send(AT24C32_ADDRESS,(uint8_t *) &ByteAddress, 2, 100);

    /* Wait to receive IIC reply from the EEP */
    while(R_IIC20_Master_Receive(AT24C32_ADDRESS, (uint8_t *) &Byte, 1, 100) != MD_OK);
